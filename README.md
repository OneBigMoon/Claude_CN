...
当前正式版本：[`v0.0.9`](https://github.com/OneBigMoon/Claude_CN/releases/tag/v0.0.9)

推荐下载：

- `ClaudeCN-macos.zip`：解压后得到 `ClaudeCN.app`，这是推荐给普通用户的菜单栏应用。
- `claude-desktop-cn-macos-m5-0.0.9.dmg`：App 型 DMG，打开后可直接运行或拖拽安装 `ClaudeCN.app`。
...
1. 下载 `Releases` 中的 `claude-desktop-cn-macos-m5-0.0.9.dmg`。
...
- 检查工作区是否干净。
- 以版本号创建/推送 `v0.0.9` 标签。
- 运行打包并将 `dist/claude-desktop-cn-macos-m5-0.0.9.dmg` 和 `dist/ClaudeCN-macos.zip` 上传到 GitHub Releases。

## Releases 建议

当前正式 Release：

- [`v0.0.9`](https://github.com/OneBigMoon/Claude_CN/releases/tag/v0.0.9)
...
建议用 `v` 打头的 tag（例如 `v0.0.9`）打包发布，这样 GitHub Releases 会按版本展示下载链接。
